...
- **All 15 review dimensions** — instruct the sub-agent to read both `references/dimensions-intra.md` (D1-D4, D6, D8, D9) and `references/dimensions-cross.md` (D5, D7, D10-D15)
...
- **MANDATORY pre-analysis instruction:** *"Before applying any review dimension, read every changed file in full and build a call graph for every public method / exported function touched by the diff. Enumerate helpers called, validations performed, state mutations executed, errors raised, and external-input paths. Output as `METHOD_CHAINS` per `references/format-single.md`. Only after producing METHOD_CHAINS may you apply dimensions. Scope note: the graph is rooted at symbols MODIFIED by the diff — not every public symbol in every changed file. Do not trust method names or helper-function purity — open and read every callee. See `references/call-graph-discipline.md` for the full protocol."*
- **MANDATORY mid-analysis instruction (pre-emission scratchpad):** *"Before writing any dimension block, build a `CANDIDATE_INVENTORY` drop ledger per `references/format-common.md` §Pre-emission Scratchpad: list ONLY the candidates you decided NOT to emit, each with a fixed-enum `drop_reason` code and a one-line `drop_detail`. Findings you keep are NEVER ledgered — they go straight into their dimension blocks. Compare each candidate against `references/suppression-gates.md` §Drop Gallery before ledgering it. An empty ledger is valid. Write the ledger BEFORE the dimension blocks."* This matters more on a public PR than anywhere else — the ledger is what forces the drop decision to happen before the finding is written, not after.
- **MANDATORY post-analysis instruction (especially important for GitHub PR mode — speculative findings on a public PR damage trust more than no findings):** *"After applying dimensions and BEFORE writing any finding, route every candidate through `references/suppression-gates.md` (Gates 1-5 — Reachability, Trust Boundary, Severity Calibration, Quota Avoidance, Factual Verifiability). Drop speculative findings ('could theoretically', 'if X ever happens'). Drop D1/D2 findings whose input source is internal/trusted under the project's threat model. Downgrade design-preference 'critical' to `warning`. Accept empty dimensions. Every `critical`/`blocker` finding MUST include a non-empty `evidence` field showing concrete reachability. The PR will be visible to the team — be conservative."*
