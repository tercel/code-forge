...
| Directory structure | ✅ Complete | docs/features/, planning/ |
...
2. **Input document path error**
   ...
   File: docs/features/xxx.md

   Suggestions:
   1. Check file path is correct
   2. Confirm file has been created
   3. Use `ls docs/features/` to see available files
   ```
```
...
git add planning/feature-x/state.json
...
If planning/ has multiple modules:
...
❌ Technical error:
```
FileNotFoundError: docs/features/xxx.md
```
